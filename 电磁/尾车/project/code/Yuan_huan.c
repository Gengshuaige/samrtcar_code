...
//    // 非圆环模式下，单边L角点, 单边长直道
//    if (circle_type==CIRCLE_NONE && Lpt0_found && !Lpt1_found && is_straight1&&check_far_whitepoint(x6,y6) ) {
//        //左角点二次检查
...
//        }
//    }
//    else if (circle_type==CIRCLE_NONE && !Lpt0_found && Lpt1_found && is_straight0&& check_far_whitepoint(x7,y7)) {
//        //右角点二次检查
//        if(Lpt1_rpts1s_id<10){
//        circle_type = CIRCLE_RIGHT_BEGIN;
//        }
//    }else if(Lpt1_found && no_left_line>2 && check_far_whitepoint(x7,y7)){
//
//        track_type = TRACK_LEFT;
//        yunahuan_right = 1;
//    }else if(Lpt0_found && no_right_line>2 && check_far_whitepoint(x6,y6)){
//        yunahuan_left = 1;
//        track_type = TRACK_RIGHT;
//    }else if(circle_type==CIRCLE_NONE && !is_straight0 && is_straight1 && yunahuan_left) {
//
//        if((GUI_ADC_DATE[1]<GUI_ADC_DATE[0]&&GUI_ADC_DATE[0]>30)){
//            circle_num_Y+=1;
...
//
//        if((GUI_ADC_DATE[3]<GUI_ADC_DATE[4]&&GUI_ADC_DATE[4]>54)) {
//            circle_num_Y+=1;
...
//        track_type = TRACK_LEFT;
//        circle_encoder += get_total_encoder();
//        //右线为长直道
//        if (circle_encoder>500) {//&& is_straight1
...
//        track_type = TRACK_RIGHT;
//
//        if (Yaw-circle_yaw<-25) { circle_type = CIRCLE_RIGHT_RUNNING; }
//    }
//    //正常巡线，寻外圆左线
...
void check_circle() {
    ...
    if(circle_type==CIRCLE_NONE && !is_straight0 && is_straight1 ) {

        if((GUI_ADC_DATE[1]<GUI_ADC_DATE[0]&&GUI_ADC_DATE[0]>50)&&(GUI_ADC_DATE[3]+GUI_ADC_DATE[4])>50){
    ...
    }else if(circle_type==CIRCLE_NONE && is_straight0 && !is_straight1 ) {

        if((GUI_ADC_DATE[3]<GUI_ADC_DATE[4]&&GUI_ADC_DATE[4]>55)&&(GUI_ADC_DATE[0]+GUI_ADC_DATE[1]>45)) {
            circle_type = CIRCLE_RIGHT_BEGIN;
            circle_yaw = Yaw;
        }
            no_left_line = 0;
...
//        bibibi(0);
    ...
    //出环，寻内圆
        ...
        //右线为长直道
        if (Yaw-circle_yaw>25) {//&& is_straight1
            circle_type = CIRCLE_LEFT_END;
        }
    }
    //走过圆环，寻右线
...
//        shexiangtou_or_dianci = true;
        //左线先丢后有
        if (rpts0s_num < 5) { none_left_line++; }
        if (rpts0s_num > 5 && none_left_line > 2&&(GUI_ADC_DATE[0]<30)) {
            circle_type = CIRCLE_NONE;
            shexiangtou_or_dianci = false;
            Element_num ++;
            circle_num_Y++;
    ...
    //出环，寻内圆
        ...
        //右线先丢后有
        if (rpts1s_num < 5) { none_right_line++; }
        if (rpts1s_num > 5 && none_right_line > 2 && GUI_ADC_DATE[4]<30) {
            circle_type = CIRCLE_NONE;
            shexiangtou_or_dianci = false;
            Element_num ++;
            circle_num_Y++;
