...
float WEIFEN_DL;
float error_away1;
int My_PID_WEIZHI_L(int Qiwang,int Shiji,float kP,float kI,float kD)//位置式
{
    error_L = Qiwang - Shiji;
    BILI_DL = error_L * kP;//计算比例项
    JIFEN_WEIZHI_L += error_L*kI;//计算积分项
    if(JIFEN_WEIZHI_L>5000)JIFEN_WEIZHI_L = 6000;
    if(JIFEN_WEIZHI_L<-5000)JIFEN_WEIZHI_L = -6000;
...
float WEIFEN_DR;
float error_away2;
int My_PID_WEIZHI_R(int Qiwang,int Shiji,float kP,float kI,float kD)//位置式
{
...
int My_PID_WEIZHI(float Shiji,int Qiwang,float L_kP,float kI,float kD)//位置式
{
    ...
    error_away = error;
    int ZUIZHONGJIEGUO = (int)(BILI + WEIFEN);// + JIFEN_WEIZHI
    return ZUIZHONGJIEGUO;
}
