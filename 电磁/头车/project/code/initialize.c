...
{
    gpio_init(D12, GPO, GPIO_HIGH, GPO_PUSH_PULL);                            // GPIO 初始化为输出 默认上拉输出高
    pwm_init(TIM4_PWM_MAP1_CH2_D13, 17000, 0);                                                  // PWM 通道初始化频率 17KHz 占空比初始为 0

    gpio_init(D15, GPO, GPIO_HIGH, GPO_PUSH_PULL);                            // GPIO 初始化为输出 默认上拉输出高
...
{
    pwm_init(TIM2_PWM_MAP1_CH1_A15, 50, 658);
