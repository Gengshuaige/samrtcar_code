...
//
//        if((GUI_ADC_DATE[1]<GUI_ADC_DATE[0]&&GUI_ADC_DATE[0]>44)){
//
...
//
//        if((GUI_ADC_DATE[3]<GUI_ADC_DATE[4]&&GUI_ADC_DATE[4]>47)) {
...
//        if (Yaw-circle_yaw>23) { circle_type = CIRCLE_LEFT_RUNNING; }//
//    }
//    //正常巡线，寻外圆右线
...
//        //外环拐点(右L点)
//        if (Lpt1_found && Lpt1_rpts1s_id < 15) {
...
//
//        //右线为长直道
//        if (Yaw-circle_yaw>25&&read_ceshi!=60) {//&& is_straight1
...
//        if (Lpt0_found && Lpt0_rpts0s_id < 15) {
//            circle_yaw = Yaw;
...
//        }
//
//        if (Yaw-circle_yaw<-40&&read_ceshi!=60) {
...
void check_circle() {
    ...
    if(circle_type==CIRCLE_NONE && !is_straight0 && is_straight1 ) {
        ...
        if((GUI_ADC_DATE[1]<GUI_ADC_DATE[0]&&GUI_ADC_DATE[0]>44)&&GUI_ADC_DATE[3]+GUI_ADC_DATE[4]>45){
        ...
        if((GUI_ADC_DATE[3]<GUI_ADC_DATE[4]&&GUI_ADC_DATE[4]>47)&&GUI_ADC_DATE[0]+GUI_ADC_DATE[1]>30) {
            circle_type = CIRCLE_RIGHT_BEGIN;
            circle_num_Y ++;
            circle_yaw = Yaw;
        }
...
//        }
        ...
        //右线为长直道
        if (Yaw-circle_yaw>30&&read_ceshi!=60) {//&& is_straight1
            circle_type = CIRCLE_LEFT_END;
        }
    }
    //走过圆环，寻右线
...
//        shexiangtou_or_dianci = true;
        //左线先丢后有
        if (rpts0s_num < 5) { none_left_line++; }
        if (rpts0s_num > 5 && none_left_line > 2&&GUI_ADC_DATE[0]+GUI_ADC_DATE[1]>30) {
            circle_type = CIRCLE_NONE;
            shexiangtou_or_dianci = false;
            Element_num ++;
            bibibi(0);
...
//        }
        ...
        //右线先丢后有
        if (rpts1s_num < 5) { none_right_line++; }
        if (rpts1s_num > 5 && none_right_line > 2&&GUI_ADC_DATE[3]+GUI_ADC_DATE[4]>45) {
            circle_type = CIRCLE_NONE;
            shexiangtou_or_dianci = false;
            Element_num ++;
            circle_yaw = 0;
