...
/**************************************************需要修改的东西*******************************************************/
enum chuku_fangxiang_e chuku_fangxiang = RIGHT;
...
void tft180_show_All(void){
    ...
    //传感器数值
//    tft180_show_float(0, 120, temp, 4 ,2);
    tft180_show_int(0, 140, Yaw, 6 );
    tft180_show_int(0, 120, Pitch, 6 );
//    tft180_show_int(0, 0, read_ceshi, 6 );
//    tft180_show_int(0, 20, circle_type, 6 );
//    tft180_show_int(0, 60, imu660ra_acc_x, 6 );
//    tft180_show_int(0, 80, imu660ra_acc_y, 6 );
//    tft180_show_int(0, 100, imu660ra_acc_z, 6 );
//    tft180_show_float(0, 90, conf, 6 , 2);
//    tft180_show_float(0, 120, qiwang_speed_L, 6 , 2);
//    tft180_show_float(0, 140, qiwang_speed_R, 6 , 2);
//    tft180_show_float(0, 110, pid_shuchu_speed_L, 6 , 2);
//    tft180_show_float(0, 130, pid_shuchu_speed_R, 6 , 2);
...
//        tft180_show_int(0, 130, track_type, 2);
//    tft180_show_float(0, 30, shiji_speed_R, 6 ,2);
//    tft180_show_float(0, 0, shiji_speed_L, 6 ,2);
//    tft180_show_float(0, 90, aim_distance, 6 ,2);
...
//      tft180_show_string(0,20,"first:");
//      tft180_show_int(71, 20, first_type, 6 );
...
//          tft180_show_int(71, 0, chongdian_low_type, 2);
//
//                tft180_show_string(0,120,"Cw_ceshi:");
//          tft180_show_int(71, 120, chongdian_low_num, 2);
...
//    tft180_show_string(0, 20, "from two:");
...
void find_zhongxian(void){
    track_leftline(rpts0s, rpts0s_num, rptsc0, (int) round(angle_dist / sample_dist), pixel_per_meter * (ROAD_WIDTH) / 2);//40.95
...
void zhongxianGuiyihua(void) {
    ...
    if (begin_id>=0 && rpts_num-begin_id>=3) {
        ...
        if(circle_type!=CIRCLE_NONE && ruku_type==RUKU_NONE) {
            ...
            if(circle_type == CIRCLE_RIGHT_IN) pure_angle_mid*=1.5;
//            if(circle_type == CIRCLE_LEFT_OUT) pure_angle_mid+=5;

        }

        pure_angle = pure_angle_mid;
//            tft180_show_float(0, 60, pure_angle, 6 ,2);
                //piancha_points(rptsn, rptsn_num);
    }
}

void Luzhang_wait(void) {
    if(read_ceshi==111) {
        wireless_ch573_send_byte(luzhang_num);
    }
}
...
//        充电完毕进行元素判别
    if(luzhang_type==LUZHANG_NONE && slope_type==SLOPE_NONE) {
        ...
        if(cross_type==CROSS_NONE && duanlu_type==DUANLU_NONE && luzhang_type==LUZHANG_NONE &&circle_type==CIRCLE_NONE&&ruku_type==RUKU_NONE)
            check_cross();
        if(circle_type==CIRCLE_NONE && duanlu_type==DUANLU_NONE)
            check_circle();
    }
        if(luzhang_type==LUZHANG_NONE && slope_type==SLOPE_NONE&& duanlu_type==DUANLU_NONE )
            check_zhangai();
    }
    }
    ...
    //控制
    if(first_type != FIRST_NONE && first_type!=CHARGE_OVER) run_chongdian3();
...
////        充电完毕进行元素判别
    if(luzhang_type==LUZHANG_NONE && slope_type==SLOPE_NONE) {
        ...
        if(circle_type==CIRCLE_NONE)
            check_circle();
        if(duanlu_type==DUANLU_NONE&&ruku_type==RUKU_NONE&&cross_type==CROSS_NONE
...
//    //控制
    if(first_type != FIRST_NONE && first_type!=CHARGE_OVER) run_chongdian3();
    if(slope_type != SLOPE_NONE) run_slope();
    if(chongdian_low_type!=CW_NONE)run_charge_low();
    if (circle_type != CIRCLE_NONE) run_circle();
    if (ruku_type != RUKU_NONE)  run_ruku();
    if (luzhang_type != LUZHANG_NONE)  {
        if(RIGHT_LZ)
